...
export function SharedHeader() {
  ...
  return (
    <>
      ...
      {/* Main Navigation */}
      <nav className="bg-white sticky top-0 z-50" style={{ boxShadow: "0 2px 20px rgba(26,35,50,0.08), 0 1px 0 rgba(197,165,114,0.3)" }}>
                  ...
                  {primaryNav.map(({ label, icon: Icon, href, testId }) => (
                    <NavigationMenuItem key={href}>
                      <button
                        ...
                        `}
                        style={isActive(href) ? { boxShadow: "0 0 15px rgba(197,165,114,0.4)" } : undefined}
                      >
                        <Icon className={`h-3.5 w-3.5 flex-shrink-0 transition-all duration-300 ${isActive(href) ? "" : "text-[#C5A572] group-hover/item:scale-110"}`} />
                        <span>{label}</span>
                      </button>
                    </NavigationMenuItem>
                  ))}
                        ...
                        <div className="grid grid-cols-2 gap-0.5">
                          {services.filter(s =>
                            ...
                            <button
                              ...
                              style={{ '--hover-bg': 'rgba(197,165,114,0.08)' } as React.CSSProperties}
                            >
